        ...
        else if(res.contains("altitude"))
        {
            ...
            if (res["transducers"].is_array() && res["transducers"].size() >= 4) {
                for (size_t beam = 0; beam < 4; beam++)
                {
                    ...
                    // Dvl.msg uses per-beam std::array<*, 4>, not scalars.
                    velocity_report.range[beam] =
                        wl_json_double(td["distance"], velocity_report.range[beam]);
                    velocity_report.beam_quality[beam] = static_cast<float>(
                        wl_json_double(td["rssi"], velocity_report.beam_quality[beam]));
                    velocity_report.beam_velocity[beam] = static_cast<float>(
                        wl_json_double(td["velocity"], velocity_report.beam_velocity[beam]));
                }
            } else {
